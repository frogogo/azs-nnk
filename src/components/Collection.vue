  ...
  >
    ...
    >
      <div class="flex flex-col">
        <h3 class="text-bold text-xl md:text-4xl ">
          {{ collection.title }}
        </h3>
        <span class="collection__description" v-if="collection.description" v-html="collection.description"/>
        <ul class="collection__list" v-if="collection.specifications">
          <li v-for="item in collection.specifications" :key="item">
            {{ item }}
          </li>
        </ul>
        <div v-if="collection.pictograms" class="flex mt-5">
          <Pictogram
            class="mr-4"
